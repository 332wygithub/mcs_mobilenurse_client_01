    ...
    export default {
        ...
        data() {
            return {
                ...
                list:[],
                //获取用户管理传递的参数
                currentRow:this.$route.params.currentRow,
                // 添加修改标识   默认为修改标识0    添加标识1
                // eventFlag:'0',
                // 科室下拉框默认绑定数据
                keshiID:'',
                //定义存储yhid的变量
        ...
        created:function () {
            if(this.currentRow != null){
                this.yhId = this.currentRow.yhId;
                this.mm = this.currentRow.mm;
                this.yhmz = this.currentRow.yhmz;
                this.sfzh = this.currentRow.sfzh;
            }
            // }
            // 有参数后再调用查询方法
            this.cxksxx();
        ...
        methods:{
            ...
            baocun:function () {
                //将修改后的数据赋给原来的值
                if(this.currentRow != null){
                    this.currentRow.yhId=this.yhId;
                    this.currentRow.mm=this.mm;
                    this.currentRow.yhmz=this.yhmz;
                    this.currentRow.sfzh=this.sfzh;
                    //把修改后的值赋给原来的关系表的值
                    this.currentRow.mcs_yhks.keshiID=this.keshiID;
                    this.currentRow.mcs_yhks.zw=this.zw;
                }
                axios
                    .put("http://127.0.0.1:8080/yhCon/tjOrxgxx",this.currentRow)
                    .then((res)=>{
                        console.log(res.data)
        ...
        watch:{
            currentRow:{
                handler(newData,oldData){
                    ...
                    // } else {
                        // 如果是修改事件 则绑定修改数据中的值
                        this.keshiID = newData.mcs_yhks.keshiID;
                        this.zw = newData.mcs_yhks.zw;
                        //取出需要绑定的信息，赋给return中定义的变量
                        if(this.currentRow != null){
                                this.yhId = this.currentRow.yhId;
                                this.mm = this.currentRow.mm;
                                this.yhmz = this.currentRow.yhmz;
